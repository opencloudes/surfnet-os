...
# Review to update with the neutron-ovs configuration to enable VPP
...
class VPPPluginContext(context.NeutronContext):
    interfaces = []
    ...
    def vpp_ctxt(self):
        ...
        if config('os-data-network'):
            ...
            try:
                vpp_ctxt['local_ip'] = get_host_ip(
                    network_get_primary_address('data')
                )
            except NotImplementedError:
                vpp_ctxt['local_ip'] = fallback

        #neutron_api_settings = NeutronAPIContext()()
        #vpp_ctxt['neutron_security_groups'] = self.neutron_security_groups
        #vpp_ctxt['l2_population'] = neutron_api_settings['l2_population']
        #vpp_ctxt['distributed_routing'] = neutron_api_settings['enable_dvr']
        #vpp_ctxt['extension_drivers'] = neutron_api_settings[
        #   'extension_drivers']
        #vpp_ctxt['overlay_network_type'] = \
        #    neutron_api_settings['overlay_network_type']
        #vpp_ctxt['polling_interval'] = neutron_api_settings['polling_interval']
        #vpp_ctxt['rpc_response_timeout'] = \
        #    neutron_api_settings['rpc_response_timeout']
        #vpp_ctxt['report_interval'] = neutron_api_settings['report_interval']
        # TODO: We need to sort out the syslog and debug/verbose options as a
        # general context helper
        #vpp_ctxt['use_syslog'] = conf['use-syslog']
        #vpp_ctxt['verbose'] = conf['verbose']
        #vpp_ctxt['debug'] = conf['debug']
        #vpp_ctxt['prevent_arp_spoofing'] = conf['prevent-arp-spoofing']
        #vpp_ctxt['enable_dpdk'] = conf['enable-dpdk']

        #net_dev_mtu = neutron_api_settings.get('network_device_mtu')
        #if net_dev_mtu:
            # neutron.conf
        #    vpp_ctxt['network_device_mtu'] = net_dev_mtu
            # ml2 conf
        #    vpp_ctxt['veth_mtu'] = net_dev_mtu

        #mappings = config('bridge-mappings')
        #if mappings:
        #    vpp_ctxt['bridge_mappings'] = ','.join(mappings.split())


        #flat_providers = config('flat-network-providers')
        #if flat_providers:
        #    vpp_ctxt['network_providers'] = ','.join(flat_providers.split())

        #vlan_ranges = config('vlan-ranges')
        #if vlan_ranges:
        #    vpp_ctxt['vlan_ranges'] = ','.join(vlan_ranges.split())
        #vpp_ctxt['nsg_log_rate_limit'] = \
        #    config('security-group-log-rate-limit')
        #vpp_ctxt['nsg_log_burst_limit'] = \
        #    config('security-group-log-burst-limit')

        #vpp_ctxt['firewall_driver'] = _get_firewall_driver(ovs_ctxt)

        #if vpp_ctxt['firewall_driver'] != OPENVSWITCH:
        #    vpp_ctxt['enable_nsg_logging'] = False
