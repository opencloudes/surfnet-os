...
def config_changed():
    ...
    for rid in relation_ids('vpp-plugin'):
        vpp_relation_joined(
            relation_id=rid,
            request_restart=request_vpp_server_restart)


@hooks.hook('vpp-api-relation-changed')
@restart_on_change(restart_map())
def vpp_plugin_api_changed():
...
def vpp_plugin_joined(relation_id=None, request_restart=False):
    secret = None
    rel_data = {
